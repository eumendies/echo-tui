...
| tool approval context | agent 请求写文件、高风险 shell 或需授权 MCP 工具前的授权状态。会话缓存优先；`manual` 直接投影现有 `choice` surface，`auto` 在缓存未命中时由单回合 resolver 调用独立 reviewer。Reviewer 使用指定 profile、带信任分区的有界用户请求与动作投影、无工具 provider 和 `reasoningEffort: none` 判断，仅精确 `yes` 产生一次性允许，其余结果回退同一人工 surface。支持 Allow once、Allow tool/session、Allow command/session、Allow all/session、Deny 和 Tell model what to do；bash 只按完全相同 command 文本复用授权 | `src/app/state/tool-approval-context.ts`、`src/app/tool-approval/`、`src/tools/tool-risk-classifier.ts` |
