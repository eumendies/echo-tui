...
| agent workflow | `/init` 和 `/review` 内置工作流。handler 把固定 prompt 通过 `submit_user_message` 交回普通用户消息流程，plan 模式下提交时自动切回 normal；definition 可声明运行级 `toolPolicy` 与 `sandboxModeOverride`，`/review` 据此启用只读边界（写工具、MCP、非只读子代理 fail-closed，沙箱收紧为 read-only）并追加说明边界已启用的本地 notice；后续走正常 agent lifecycle | `src/commands/agent-workflows/*` |
...
| MCP manager | 启动期从共享配置 snapshot 初始化 enabled MCP servers，单 server 失败只记录脱敏诊断；以 `mcp__<server>__<tool>` 命名空间暴露工具、按名调用、保存 per-tool approval、支持重载与关闭。`/mcp` 保存后消费 writer 已安装的 revision 并显式 reload 一次；外部 watcher 变化只更新 Context，不隐式关闭或连接 MCP 进程 | `src/mcp/manager.ts`、`src/mcp/client.ts`、`src/config/mcp-config.ts` |
