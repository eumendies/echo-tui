...
| App settings | `~/.echo/config.json` 的 `instructions.fileName`（`AGENTS.md` / `CLAUDE.md`）、`compaction.thresholdRatio`（0.5–0.95）、`tools.readFiles.autoCompressImages`、`tools.approval.mode/modelProfileId` 和 `ui.slashSuggestionMaxVisible`（1–20）、`ui.showReasoningSummary` 逐字段归一化并缓存到 `AppContext`；普通 render 热路径只读缓存。审批策略按 assistant turn 快照，与 interaction mode 正交，配置变化只影响下一回合；reasoning 开关只过滤可见 transcript block，不删除 journal/provider 事实 | `src/config/app-settings-config.ts`、`src/app/state/app-context.ts`、`src/types/render.ts` |
