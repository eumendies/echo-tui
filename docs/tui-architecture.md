...
| 用户 memory | 用户显式维护的全局持久背景，存储于 `~/.echo/memories.json`；`/memory` 支持新增、编辑、Space 启停和确认删除。每轮真实 provider 请求重新读取并 transient 注入已启用项，绝不写入 transcript、session 或 compaction；其 token 在 `/context` 中单独显示，内容会发送给 provider，不能当作秘密存储 | `src/memory/memory-store.ts`、`src/commands/memory-command-handler.ts` |
