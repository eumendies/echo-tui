...
| agent loop runtime | provider-neutral 的真实 agent 编排层位于同一 `loop-runtime` 包，但主 Agent 与子 Agent使用独立业务入口。主 runtime 在 assistant turn 启动时捕获配置 snapshot，再从同一 revision 解析 provider、reasoning、完整 tool registry、context window、压缩阈值、指令文件名和 skill catalog 预算，并负责 Todo、用户提问、MCP、普通审批、主 transcript callback和 `run_subagent` Port。每次已接受的委派都由 factory 创建新的 subagent runtime；子入口拥有自己的 continuation loop、工具策略、hook和专属 callback协议，复用父 revision、cwd、项目指令、memory和 skills快照，但只从委派任务开始。Explorer固定使用只读allowlist、MCP禁用和人工Bash升级；Worker拥有独立Todo与除再次委派外的完整工具目录，可复用共享MCP manager并继承父normal/plan和headless策略。父子不共享业务 loop、runtime实例、provider continuation、record region、compaction、Todo或registry；只共享 provider context、usage调试投影等无角色纯函数和进程级服务 | `src/agent/loop-runtime/agent-loop-runtime.ts`、`src/agent/loop-runtime/subagent-loop-runtime.ts`、`src/agent/loop-runtime/shared.ts`、`src/agent/subagent/` |
...
| tool message rendering | app 可见层把未完成 tool call 显示为 footer pending preview；需要用户参与的工具先进入 approval 或 user-question surface；tool result 到达后再追加具备必填 call id/name/status 和 discriminated `details.kind` 的 `tool_call` / `tool_result` records。顶层 renderer 负责路由和通用 fallback，子目录分别承载 bash、apply_patch、use_skill 及共享宽度/前缀/rail 行逻辑。Subagent renderer 按 runId 生成外层 rail，在扣除宽度后复用内部工具行级 renderer；完整委派任务不受普通工具显示行数预算截断，实际Agent标题、任务文本和最外层marker/rail统一使用专属`subagentRail`主题色，内部工具标题、状态、prefix、正文以及reasoning/assistant/阶段文本全部映射到`toolOutput`。每个稳定工作事件及transient活动前插入不断开的外层rail空行。实时call在footer中保持瞬时投影，result到达后成对写入历史区；稳定区与footer共用rail几何，活动草稿由统一timer刷新。恢复时缺少终态的run显示意外中断，外层pair按参数动态显示Explorer或Worker紧凑状态。`use_skill` 成功结果只投影为简洁使用摘要，agent memory 脚本按普通 bash rail 展示，旧 memory tool records 按通用 fallback 展示 | `src/app/state/turn-context.ts`、`src/render/tool-message-renderer.ts`、`src/render/subagent-renderer.ts`、`src/render/tool-message-renderers/` |
...
| `src/agent/loop-runtime/subagent-loop-runtime.ts` | `createSubagentLoopRuntime` | 定义绑定的Subagent runtime：隔离任务、独立registry/Todo/continuation、Explorer只读或Worker通用工具策略、hook和专属callbacks |
