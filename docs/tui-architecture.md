...
| App settings | `~/.echo/config.json` 的 `instructions.fileName`（`AGENTS.md` / `CLAUDE.md`）、`compaction.thresholdRatio`（0.5–0.95）和 `ui.slashSuggestionMaxVisible`（1–20）、`ui.showReasoningSummary` 逐字段归一化并缓存到 `AppContext`；普通 render 热路径只读缓存。reasoning 开关只过滤可见 transcript block，不删除 journal/provider 事实；指令文件选择和压缩阈值在每次 agent run 启动时形成快照并贯穿 tool continuation | `src/config/app-settings-config.ts`、`src/app/state/app-context.ts`、`src/types/render.ts` |
