...
| MCP manager | 启动期从共享配置 snapshot 初始化 enabled MCP servers，单 server 失败只记录脱敏诊断；以 `mcp__<server>__<tool>` 命名空间暴露工具、按名调用、支持重载与关闭。没有 server 级审批开关：审批边界只由工具 `annotations.readOnlyHint` 归一出的事实决定——声明只读的工具免审批，hint 缺失或为 false 时一律进入审批；同一事实也提供只读运行的 MCP 准入判定。`/mcp` 保存后消费 writer 已安装的 revision 并显式 reload 一次；外部 watcher 变化只更新 Context，不隐式关闭或连接 MCP 进程 | `src/mcp/manager.ts`、`src/mcp/client.ts`、`src/config/mcp-config.ts` |
...
| `src/tools/tool-risk-classifier.ts` | `classifyToolCallRisk`、`classifyReadonlyToolCall`、`parseBashCommand` | 执行前策略分类：安全执行、请求审批或按 mode 直接拒绝，覆盖 apply_patch、高风险 bash 与 MCP 只读注解边界；plan 与 readonly 分支接受 bash 沙箱生效标志，生效只读沙箱下 bash 交内核边界，未生效回退严格只读 allowlist；普通与只读运行的 MCP 分支都只放行 server 声明 `readOnlyHint` 的工具，其余进入审批或（只读运行）以专用文案 fail-closed |
