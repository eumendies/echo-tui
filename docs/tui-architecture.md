...
flowchart LR
    ...
    app_main --> user_config_context["src/config/user-config-context.ts"]
    ...
    app_main --> agent_loop["src/agent/loop-runtime/agent-loop-runtime.ts"]
    ...
    agent_loop --> user_config_context
    user_config_context --> llm_config["src/config/llm-config.ts"]
    user_config_context --> app_settings["src/config/app-settings-config.ts"]
    user_config_context --> mcp_config["src/config/mcp-config.ts"]
    user_config_context --> hook_config["src/hooks/config.ts"]
    ...
    agent_loop --> subagent_runtime["src/agent/subagent/*"]
    subagent_runtime --> run_subagent_tool["src/tools/run-subagent-tool-handler.ts"]
    ...
    app_context --> subagent_context["src/app/state/subagent-run-context.ts"]
    ...
    app_main --> auto_update["src/app/auto-update-controller.ts"]
    app_main --> update_modules["src/update/*"]
...
`src/app/main.ts` 是生产 composition root，面向一个 `AppContext` 组合根；`AppContext` 创建并以只读引用公开 `ComposerContext`、`TranscriptContext`、`ModelContext`、`TurnContext`、`RenderContext`、`ChangeHistoryContext`、`ConversationReferenceContext`、`PendingMessageContext` 和 `SubagentRunContext`，内部持有 `SlashSuggestionContext` 以及 interaction mode、context usage、MCP bootstrap 状态等进程级标量状态。调用方直接使用状态所属的子 context；`AppContext` 负责派生 render/agent 组合状态，并协调 session 加载与清空、change checkpoint、undo、user/assistant turn 等跨 context 事务。`SubagentRunContext` 以 runId 为键保存每个并发子 Agent 的 run identity、阶段、草稿、内部工具摘要、并行标记和耗时锚点，稳定过程仍由主 transcript 持有，且不会覆盖父 `TurnContext` 的 pending tool。`ToolApprovalContext`、`UserQuestionContext` 和 `FilePickerContext` 由 `main.ts` 直接持有；启动更新流程由 app 层 `AutoUpdateController` 承载。composer、transcript records、session 指针、response lock、assistant pending、单槽待发送消息、working spinner、输入历史、slash suggestion、change history、status line 和 previous terminal size 由对应 context 或 render state 持有；boxed composer 的 placeholder 只在 render 层生成。`main.ts` 显式装配 terminal、renderer、stores、command runtime、agent runtime 和三个粗粒度 controller，并保留 render/append、shell execution、assistant interruption、MCP/config lifecycle、resize、start、exit，以及 `AutoUpdateController` 的检查/更新/持久化端口装配。`ComposerSubmissionController` 负责 composer 消费、pending dispatch 以及 command/skill/shell/file mention/reference 提交路由；它只把最终 `AssistantTurnSubmission` 交回 main 装配的真实运行边界。`InputEventController` 持有 stateful key parser，按固定优先级协调 modal、subagent 会话窗口、local surface、快捷键、composer 和 Esc/Submit/Exit 动作。普通 assistant turn 与子 Agent callback 到状态的翻译由 `src/app/assistant-turn-runner.ts` 承载。普通状态变化统一调用 `app-renderer.render`，该入口先按需提交流式稳定前缀再重绘 footer；transcript append 与 destructive replay 继续使用各自的显式入口。slash command session 和会话内事件分发由 `src/app/command/command-runtime.ts` 承载；`src/app/command/command-host.ts` 在组合根装配领域 command ports。

`src/app/main.ts` 同时为每个 TUI 实例创建唯一 `UserConfigContext`。Context 对 `~/.echo/config.json` 的一次读取和 JSON 解析形成不可变 revision snapshot；App settings、LLM/provider、tool runtime、MCP、hooks 和配置草稿 selector 都从该 snapshot 工作，不再各自访问文件。`ModelContext` 只接收模型目录与 runtime resolver port，不拥有磁盘边界；agent runtime 和 `McpManager` 同样要求显式注入配置 port。TUI watcher 检测到语义变化后发布一个带领域 change set 的新 revision；`main.ts` 订阅者统一应用 App/Model snapshot、清理失效 usage 并选择 footer 或 destructive redraw。MCP 和 hooks 保持显式 reload 边界，不因外部 watcher 变化自动执行进程 reload。除 TUI 与 headless composition root 外，生产模块不得自行创建 `UserConfigContext`。
...
| 用户配置上下文 | 每个 TUI/headless 实例集中读取 `~/.echo/config.json`，以稳定语义 fingerprint 去重并发布不可变 revision；同一 revision 的 runtime selector 返回冻结值，配置草稿返回隔离副本。writer 总是基于磁盘最新根做领域增量修改和原子 rename，成功后同步安装 revision；后续 watcher 的相同内容事件不会重复通知或重做副作用 | `src/config/user-config-context.ts` |
| interaction mode | 当前进程内的交互模式，含 `normal`、`plan`、`shell`、`shell-local`。Tab 在四者间循环，`/mode` 也可切换；normal/plan 的模型可见切换只包装到切换后首条 user record，plan 同时影响工具风险边界并在下一次 default 工具策略运行派生运行级 `read-only` 沙箱收紧，shell/shell-local 把输入作为本地 bash 命令执行 | `src/app/state/app-context.ts`、`src/types/agent.ts` |
...
| input event controller | 持有跨 stdin chunk 的 key parser，按固定优先级分发 modal、subagent 会话窗口、command/local surface、快捷键、composer、Esc、Submit 和 Exit；业务收尾通过 main 装配的必填 action 调用 | `src/app/input-event-controller.ts` |
| startup update check | TUI 首帧渲染后异步查询 npm registry 的 `@eumendies/echo-tui` latest 版本并与当前版本比较；源码运行、npx 缓存与关闭开关直接跳过，失败静默降级，成功检查按 24h TTL 缓存到 `~/.echo/update-state.json`。`AutoUpdateController` 承载检查结果、呈现等待与提示状态机，由既有 activity timer 的 tick 驱动、以最低优先级 modal 承载「更新可用」choice 卡片（立即更新 / 稍后提醒 / 忽略此版本）；`main.ts` 只注入检查/更新/持久化端口与空闲门控闭包。「立即更新」先完成终端收尾，再由 `src/update/update-runner.ts` 前台执行 `npm install -g @eumendies/echo-tui@latest` 并重启新版本，npm 失败时以当前版本重启；重启前写入状态文件短窗口标记，避免重启进程立即重复提示 | `src/update/*`、`src/app/auto-update-controller.ts`、`src/app/main.ts` |
...
| agent workflow | `/init` 和 `/review` 内置工作流。handler 把固定 prompt 通过 `submit_user_message` 交回普通用户消息流程，plan 模式下提交时自动切回 normal；definition 可声明运行级 `toolPolicy` 与 `sandboxModeOverride`，`/review` 据此启用只读边界（写工具、未声明只读的 MCP 工具、非只读子代理 fail-closed，沙箱收紧为 read-only）并追加说明边界已启用的本地 notice；后续走正常 agent lifecycle | `src/commands/agent-workflows/*` |
| slash suggestion context | 普通 composer 输入态的 slash 命令提示状态。基于 handler 元数据、enabled skill 与 MCP prompt 命令派生候选项，处理 Up/Down 循环选择和 Tab 补全，不启动 command session | `src/app/state/slash-suggestion-context.ts` |
...
| tool approval context | agent 请求写文件、高风险 shell 或需授权 MCP 工具前的授权状态。会话缓存优先；`manual` 直接投影现有 `choice` surface，`auto` 在缓存未命中时由单回合 resolver 调用独立 reviewer。Reviewer 使用指定 profile、带信任分区的有界用户请求与动作投影、无工具 provider 和 `reasoningEffort: none` 判断，仅精确 `yes` 产生一次性允许，其余结果回退同一人工 surface。支持 Allow once、Allow tool/session、Allow command/session、Allow all/session、Deny 和 Tell model what to do；bash 只按完全相同 command 文本复用授权。主 Agent和子 Agent共享同一缓存与审批流程；子 Agent origin只用于 surface标题和迟到请求隔离，不改变授权选项或缓存命中；并发子 Agent 审批请求按 FIFO 排队逐个呈现，活跃请求决议后自动提升下一个排队请求，父 turn 中断时活跃与排队请求统一以 interrupted 收尾 | `src/app/state/tool-approval-context.ts`、`src/app/tool-approval/`、`src/tools/tool-risk-classifier.ts` |
...
| transcript store | 按 cwd hash 分区的本地 append-only JSONL session journal。默认根目录是 `~/.echo/echo_tui/`；每个 session 以 `session_start` 首行建立身份，随后逐行追加 records、compaction、todo 和 `/undo` / `/diff` 共用的 change history 操作。子 Agent 的 start、稳定文本、内部工具和终态继续使用普通 `append_records` 增量落盘；预览把连续同 run 过程压成单条摘要。同目录 `{session-id}.settings.json` sidecar 原子覆盖当前 model profile ID 和可选 effort override，不保存 profile 定义或历史值；孤立 sidecar 不进入 session 枚举。journal 重放校验 envelope、record role 和下游依赖的关键身份字段，包括 subagent run/parent call identity 与事件结构，但保留未知扩展字段；display metadata 等可选 payload 由消费端自行校验。重放忽略末尾未完成写入，真正恢复 session 时会原子移除无效尾部后再允许续写，但拒绝中间损坏；不保存 composer、pending、command session 或输入历史 | `src/persistence/transcript-store.ts`、`src/persistence/transcript-journal.ts`、`src/persistence/session-model-settings-store.ts` |
...
| change history context | assistant loop 的 change checkpoint 栈，记录受控文件 snapshot 快照和 `pending` / `created` / `updated` 写入状态；同一份 history 同时服务 `/undo` 回退和 `/diff` fallback。不可追踪写入型 bash 使 checkpoint 失效，但在生效 `read-only` 沙箱内执行的命令不失效（判定与执行包装同源） | `src/app/state/change-history-context.ts` |
| LLM config | 从当前用户配置 revision 解析必填 `llm.providers`、`llm.models`、可选 `llm.selectedModel` / `contextWindow` / `reasoning` 配置；`llm.selectedModel` 与 profile effort 是新 session 默认值，交互式当前值由 session sidecar 持有。model profile 通过 `provider` 引用 provider profile，provider profile 通过 `preset` 引用内置 provider preset catalog，解析层把有效选择归一化为单个运行时 `{agentType, apiKey, baseURL, codexOAuth, headers, model, reasoningEffort, reasoningSummary, contextWindow, tools}` 配置；单次运行可传入 model/effort override，不写回全局配置。用户配置不暴露 `agentType`，协议类型由 preset 后台解析；`openai-codex-oauth` preset 不要求 `apiKey`，只记录可选 `codexAuthFile`，运行时按 `codexAuthFile` → `CODEX_HOME/auth.json` → `~/.codex/auth.json` 读取既有 Codex OAuth cache；context window 按用户配置 → 内置模型映射 → 默认窗口三级回退 | `src/config/user-config-context.ts`、`src/config/llm-config.ts`、`src/config/provider-presets.ts`、`src/config/codex-oauth.ts` |
| App settings | 当前用户配置 revision 的 `instructions.fileName`、压缩阈值、tool runtime 和 UI/审批设置由 snapshot selector 逐字段归一化并在 revision 内缓存。`AppContext` 只保留上次已应用 snapshot 引用用于分类重绘与 usage 失效副作用；render、提交准备和 assistant turn 直接读取当前或本回合固定 snapshot，不保存第二份完整 settings。外部编辑经 watcher 的短延迟发布，已启动 turn 固定旧 revision，下一 turn 才使用新模型、reasoning、tools 和 App runtime settings。malformed 配置会安装可诊断的无效 source state，而非混用上一个有效 revision；reasoning 开关只过滤可见 transcript block，不删除 journal/provider 事实 | `src/config/user-config-context.ts`、`src/config/app-settings-config.ts`、`src/app/state/app-context.ts` |
| MCP manager | 启动期从共享配置 snapshot 初始化 enabled MCP servers，单 server 失败只记录脱敏诊断；以 `mcp__<server>__<tool>` 命名空间暴露工具、按名调用、支持重载与关闭。对声明 `resources` capability 的 server，启动期同时按分页与条目上限拉取并缓存资源与模板目录，暴露 `listResources`/`listResourceTemplates`/`readResource`；资源端点失败只降级为空目录并记录脱敏诊断，不影响工具能力。声明 `prompts` capability 的 server 同样在启动期缓存有界 prompt 目录，并在 `notifications/prompts/list_changed` 后原地刷新（不重连、不中断运行），暴露 `listPrompts`/`getPrompt`。没有 server 级审批开关：审批边界只由工具 `annotations.readOnlyHint` 归一出的事实决定——声明只读的工具免审批，hint 缺失或为 false 时一律进入审批；同一事实也提供只读运行的 MCP 准入判定。`/mcp` 保存后消费 writer 已安装的 revision 并显式 reload 一次；外部 watcher 变化只更新 Context，不隐式关闭或连接 MCP 进程 | `src/mcp/manager.ts`、`src/mcp/client.ts`、`src/config/mcp-config.ts` |
...
| agent loop runtime | provider-neutral 的真实 agent 编排层位于同一 `loop-runtime` 包，但主 Agent 与子 Agent使用独立业务入口。主 runtime 在 assistant turn 启动时捕获配置 snapshot，再从同一 revision 解析 provider、reasoning、完整 tool registry、context window、压缩阈值、指令文件名和 skill catalog 预算，并负责 Todo、用户提问、MCP、普通审批、主 transcript callback和 `run_subagent` Port。每个主运行从内置定义、用户级 `~/.echo/agents/*.md` 和项目级 `.echo/agents/*.md` 生成一次冻结 catalog；项目同名定义优先，内置名称不可覆盖，严格 manifest 只能在 readonly/general 能力上限内收窄本地工具及 MCP。自定义 manifest 可严格引用同一 snapshot 的 model profile，并以 inherit/default/固定值表达 effort；失效引用不会宽松回退。内置 Explorer/Worker 的非安全 model/effort/Skills override 来自用户级或项目级 `agents.settings.json`，项目条目整体遮蔽用户条目且不能改变内置 prompt、工具或执行策略。schema、执行解析和 debug 诊断共享该快照，目录缺失按空目录处理，运行中修改下次运行才生效。每次已接受的委派都由 factory 创建新的 subagent runtime；子入口拥有自己的 continuation loop、工具策略、hook和专属 callback协议，复用父 revision、cwd、项目指令、memory和未裁剪的 Skill snapshot，但只从委派任务开始。子运行在最终模型解析后按定义三态 Skill 策略（缺省=全部 enabled；显式空=不加载任何 Skill；非空 allowlist=仅列出的 enabled Skill，stale 名称保留展示但不进入 catalog）从同源 snapshot 创建子运行专属 scoped Skill registry，并按子模型 context window 重新计算 catalog 提示预算；并行与连续委派共享不可变父 snapshot但隔离 registry、已加载正文和 provider continuation，无 use_skill 工具的 Agent 注册空 registry。Skill allowlist 只收窄 use_skill 可见范围，不改变工具、MCP、审批、plan/headless 与递归委派边界，也不是文件路径保密边界——读取工具和 Bash 仍可访问磁盘上的 Skill 文件。内置 Explorer 与自定义 readonly 在 default 父 run 下使用只读策略、MCP禁用和未知 Bash 人工升级；父 run 声明 readonly 时子运行继承同一 fail-closed 只读边界与沙箱收紧（生效只读沙箱下任意 Bash 交内核边界，否则仅严格 allowlist 与直接拒绝），MCP 只读工具名称集合在 run 启动时由 manager 工具目录固定（仅 `readOnlyHint` 工具），普通审批与只读运行准入共用，运行中不重新读取；内置 Worker 与自定义 general 使用通用策略、独立Todo、声明后的工具目录及可选共享MCP，并继承父normal/plan和headless策略：plan 父运行派生的 `read-only` 收紧经委派端口下传，Worker 用同一 resolver 重算 bash 分层，两分支表现与主 Agent 一致。父子不共享业务 loop、runtime实例、provider continuation、record region、compaction、Todo或registry；只共享 provider context、usage调试投影等无角色纯函数和进程级服务 | `src/agent/loop-runtime/agent-loop-runtime.ts`、`src/agent/loop-runtime/subagent-loop-runtime.ts`、`src/agent/loop-runtime/shared.ts`、`src/agent/subagent/` |
| Agent 管理存储 | `/agents` 同时投影内置定义、用户级与项目级物理 Markdown 文件、来源覆盖和结构化诊断。管理存储只从 scope 与合法名称构造目标路径，拒绝保留名、符号链接和非普通文件；创建使用排他语义，更新/删除使用 SHA-256 内容指纹，写入通过同目录临时文件原子替换。无效文件保留在物理视图中供诊断或删除，成功修改只影响下一 primary assistant run | `src/agent/subagent/management-store.ts`、`src/agent/subagent/safe-storage.ts`、`src/agent/subagent/settings.ts` |
| headless 单轮 runner | `--once` 创建一个不启动 watcher 的 `UserConfigContext`，让 MCP、hooks 和 agent 共用同一 snapshot；不创建 raw mode、renderer、stdin listener、transcript session 或 session settings sidecar。默认拒绝审批工具，`--full-access` 仅放宽当前调用；成功只写最终纯文本，结束时关闭 Context、MCP/debug | `src/cli/one-shot.ts`、`src/config/user-config-context.ts`、`src/types/agent.ts` |
| lifecycle hooks | 当前用户配置 snapshot 的 `hooks` 可配置本地旁路命令；结果不显示、不写 transcript、不回传模型。`/hooks` 保存后只替换 `hooks` 节点并显式 live reload dispatcher；外部 watcher 的 hooks revision 变化不会自动执行进程配置 | `src/hooks/*`、`src/types/hooks.ts` |
| provider agent setup | 按调用方传入的已解析 `LlmConfig` 或配置 snapshot 选择 provider adapter 并装配 tool registry，不自行读取用户配置；Codex OAuth token 仍按独立凭据生命周期解析和刷新 | `src/agent/agent-setup.ts`、`src/agent/codex/agent.ts`、`src/config/codex-oauth.ts` |
| tool message rendering | app 可见层把未完成 tool call 显示为 footer pending preview；需要用户参与的工具先进入 approval 或 user-question surface；tool result 到达后再追加具备必填 call id/name/status 和 discriminated `details.kind` 的 `tool_call` / `tool_result` records。Esc 中断时由状态层为已播报但未取得 result 的调用在同一 journal batch 内补写相邻 `tool_call` 与合成失败 `tool_result`，renderer 复用既有 pair-aware 投影显示失败状态，不自行伪造结果。顶层 renderer 负责路由和通用 fallback，子目录分别承载 bash、apply_patch、use_skill 及共享宽度/前缀/rail 行逻辑。Subagent renderer 按 runId 生成外层 rail，在扣除宽度后复用内部工具行级 renderer；完整委派任务不受普通工具显示行数预算截断，实际Agent标题、任务文本和最外层marker/rail统一使用专属`subagentRail`主题色，内部工具标题、状态、prefix、正文以及reasoning/assistant/阶段文本全部映射到`toolOutput`。每个稳定工作事件及transient活动前插入不断开的外层rail空行。实时call在footer中保持瞬时投影，result到达后成对写入历史区；稳定区与footer共用rail几何，活动草稿由统一timer刷新。恢复时缺少终态的run显示意外中断；外层pair保留Explorer/Worker专属成功文案，自定义合法名称显示通用completed/failed身份。rail、footer详情、审批和提问标题均通过共享名称校验，自定义名称保留目录原名，损坏或含控制字符身份回退为Subagent。带`parallelSize`标记的并行运行不进入主窗口rail投影，改由footer plural紧凑块与Ctrl+O会话窗口承担实时可见性。`use_skill` 成功结果只投影为简洁使用摘要，agent memory 脚本按普通 bash rail 展示，旧 memory tool records 按通用 fallback 展示 | `src/app/state/turn-context.ts`、`src/render/tool-message-renderer.ts`、`src/render/subagent-renderer.ts`、`src/render/tool-message-renderers/` |
...
| file edit tools | `tools.fileEdit.mode` 每轮只选择 `apply_patch`（默认）或 `edit_file`。前者仅接收 `*** Begin Patch` Add/Update/Delete File 格式，不接收独立 unified diff；后者只更新已有 UTF-8 文本文件，以精确 `old_string`/`new_string` 执行唯一匹配或显式 `replace_all` 的原始内容非重叠匹配。两者都先模拟、复用 change recorder，并生成可持久化的共享 diff display metadata；renderer 负责上下文折叠和预算，不重新读取目标文件。`/diff` 使用独立的 Git unified diff 解析与展示，不受文件编辑输入语法变更影响 | `src/tools/apply-patch-tool-handler/`、`src/tools/edit-file-tool-handler/`、`src/app/diff/` |
...
| use_skill tool | 默认 skill 指令读取工具。加载 enabled skill 的 `SKILL.md` frontmatter 与正文，并在存在附加资源时追加 `[Skill Resources]` 清单；在子 Agent 中 registry 是按定义三态 allowlist 从父 snapshot 投影的 scoped 视图，加载范围与该子运行的 catalog 一致；disabled/missing skill 返回失败和可用 skill 列表。该工具不读取或应用 skill model override，普通 turn 和 slash override turn 的 continuation 都保持初始化模型 | `src/tools/use-skill-tool-handler.ts`、`src/skills/skill-manager.ts` |
...
| MCP resource tools | 默认 MCP 资源读取工具，仅在至少一个 MCP server 初始化成功时随共享 manager 注册进默认 registry（未配置或全局关闭 MCP 时保持仅内置工具目录），并受 `allowedToolNames` 收窄：`list_mcp_resources` 按可选 server 过滤列出资源与 templates，`read_mcp_resource` 按 server 与 uri 读取并把多条 contents 按序合并。文本沿用 20,000 bytes 预算与 head offloading，blob 只投影 mimeType 与字节数占位符；两者都是只读观察工具，直接判安全并与其它只读调用并行调度 | `src/tools/mcp-resource-tools.ts` |
...
| OpenAI Responses agent | 单次 provider turn adapter。基于 OpenAI SDK 构造 Responses request，读取 stream，按 part 顺序实时上报可读 reasoning summary 完整 preview；`response.reasoning_summary_text.done` 更新对应 part，`response.output_item.done` 可用完整 item 校正 preview，二者都不触发 summary record。整个 provider turn 成功收到 `response.completed` 后才发出一次权威 complete。adapter 返回 assistant draft、provider-private reasoning records（仅带 `encrypted_content` 时回传）与 provider-neutral tool calls；不在 turn result 中重复携带可见 summary，不执行本地工具循环 | `src/agent/openai-responses/agent.ts`、`src/agent/openai-responses/transcript-converter.ts` |
...
| `status` | 为 `/status` 聚合 cwd、AGENTS 来源、有效 memory、model/provider 与 session id；沙箱行按当前 interaction mode 解析（plan 展示派生的 `read-only` 收紧）；Codex provider 额外复用 OAuth 凭据查询账户配额，失败只返回脱敏不可用状态 |
...
自动测试覆盖 revision、writer、MCP/hooks 和 headless 边界；交互式终端交付时再人工确认以下行为：

1. 启动 TUI 后外部编辑 `~/.echo/config.json`，确认 watcher 延迟后 `/status`、footer 模型和常规设置更新；只改空白或缩进不应产生额外可见刷新。
2. 在 `/config` 分别保存常规设置和模型设置，确认保存后立即可见，当前 session 的有效模型选择保持不变；删除当前 profile 时应回退全局默认。
3. 用 `/model` 和 `/effort` 切换 session 选择，再打开 `/status`，确认三处展示一致且配置文件中的全局默认未被改写。
4. 在 assistant streaming 或 tool continuation 期间外部修改模型、reasoning、file edit tool 或压缩设置，确认当前 turn 保持旧配置，下一 turn 才采用新 revision。
5. 在 `/mcp` 启停 server，确认保存后只 reload 一次且后续工具列表更新；外部编辑 MCP 节点不应自动重连，重新进入并显式保存后再生效。
6. 在 `/hooks` 保存配置，确认 dispatcher 立即使用新配置；外部编辑 hooks 节点不应自动 reload dispatcher。
7. 把 `config.json` 临时改成 malformed JSON，确认 App 可选设置按默认值降级、LLM 请求给出脱敏配置错误；修复文件后下一 revision 恢复。
8. 运行 `echo-tui --once "..."` 和 `echo-tui --once --full-access "..."`，确认不进入 raw mode、不启动 watcher、默认拒绝需审批工具，且最终 stdout 只有纯文本结果。
...
flowchart TB
    ...
    parse --> modal{"approval / question / file picker / update prompt / mcp 诊断活跃?"}
...
输入分发的优先级是：user question → tool approval → file picker → update prompt → subagent 会话窗口 → active command session → conversation reference preparation → reference/MCP 本地 info surface → model tuning → 全局快捷键 → `@` 触发 file picker → slash suggestion → Tab 模式循环 → composer 编辑 → 历史浏览 / 换行 / Esc 中断 / 提交 / 退出。Ctrl+O 在无 command session 时打开 subagent 会话窗口；窗口活跃时 Esc/↑/↓/Ctrl+O 由窗口消费，Esc 只关闭窗口且不触达中断路径，Exit 仍走退出。`InputEventController` 使用 `createKeyParser()` 创建的 stateful parser 解析 stdin chunk：普通按键仍复用 `parseKeyChunk()` 的无状态解析，bracketed paste 则跨 chunk 缓存起止标记和 payload，把 CR/CRLF 归一为 LF 后作为单个 `TEXT` 事件交给 composer，避免粘贴中的换行被误判为提交。提交由 `ComposerSubmissionController` 统一消费；若未命中命令且处于 shell/shell-local 模式，则通过 main 的 shell action 执行，而不是发给模型。Esc 在普通输入层依次清理 pending message、conversation reference，再尝试中断 shell 和 assistant turn。更新提示由既有 activity timer 的每个 tick 尝试呈现，且只在 MCP bootstrap 完成、无本地错误 surface、composer 为空且最近约 1 秒没有输入时弹出；选择「立即更新」后 app 先完成终端收尾，再由 `src/update/update-runner.ts` 前台运行 `npm install -g @eumendies/echo-tui@latest` 并以同一入口脚本重启，npm 失败时改用当前版本重启。
...
flowchart TB
    ...
    tool_call --> risk{"风险分类"}
    risk -->|apply_patch/高风险 bash/需授权 MCP| approval_cache{"命中会话授权?"}
    approval_cache -->|是| execute_or_deny
    approval_cache -->|否 manual| approval["tool approval choice surface"]
    approval_cache -->|否 auto| auto_review{"独立模型精确返回 yes?"}
    auto_review -->|是| execute_or_deny
    auto_review -->|否/失败| approval
    ...
    interrupt --> abort["abort 当前 turn, 隔离迟到回调, 补齐未完成 tool pair, 提交 partial + 本地中断 notice"]
    abort --> finalize
...
response 期间不会启动第二次提交。每个普通 turn 开始时先创建 change checkpoint（记录 transcript 边界、compaction 状态和受控文件 snapshot），结束（完成、失败或中断）时 finalize 并持久化，供 `/undo` 和 `/diff` 使用。Esc 中断当前 assistant response：app abort 当前 turn、停止接受该 turn 的 stream/tool 回调、把完整当前 reasoning/assistant draft 落成 partial records；已播报但尚未取得 result 的 tool call 会在同一个 journal batch 内成对补齐（相邻 `tool_call` 加 `ok: false` 的合成 `tool_result`，文案说明该调用被用户中断且未返回结果），再追加本地中断 notice，避免模型已发起的调用从 transcript 中消失，同轮尚未播报的调用不补写；已经焊入 terminal scrollback 的稳定前缀只作为这些 record 的 in-flight projection，不会重复输出。迟到回调被 turn token 隔离。thinking spinner 表示首字响应前的等待；首个 assistant token 到达后切换为 working spinner 并显示已耗时。

### 内置 Subagent 边界

主 Agent 的 `run_subagent` 是普通 `ToolHandler`，通过 `ToolRegistry` / `ToolExecutor` 和 run-scoped `SubagentToolPort` 同步执行。内置定义目录是各子 Agent name、description、prompt、本地工具集合、MCP可见性和执行策略的唯一来源；handler通过Port列出的name/description动态生成必填`agent` enum及参数目录，顶层工具description保留通用委派语义；并发分类器把参数可解析且命中 readonly 谓词的 `run_subagent` 分类为 parallel_read，解析失败或 general 目标独占，并行段规模经 `subagentGroupSize` 下行并写入 start record `parallelSize`。调用时Port按名称取得完整定义，并把定义交给factory创建新的专用subagent runtime；`SubagentLoopInput` / `SubagentLoopCallbacks`只交付任务、身份、取消、mode和活动协议，不伪造主`AgentSessionInput`，也不递归复用父`RunAgent`。外层handler使用`pair_after_execute` commit mode：Port把子callback翻译出的稳定`subagent` records先提交到父runtime record region和主session journal，子运行结束后才成对提交外层`run_subagent` call/result。目录包含只读调查`explorer`和通用执行`worker`；二者都不继承父transcript、Todo、compaction或journal，只继承父run已捕获的配置revision、cwd、适用AGENTS指令、memory和skill catalog，并受单层嵌套限制；同一并行段内的多个 readonly `run_subagent` 委派并发执行（无并发上限），general 委派保持独占，父级取消传播到全部子运行。

`explorer` 的 provider-visible registry 和执行 registry 都只包含 `read_files`、`glob`、`grep`、`run_bash_command`、`web_fetch`、`web_search`、`use_skill` 以及 MCP 资源读取工具 `list_mcp_resources`、`read_mcp_resource`，不包含写文件、Todo、提问、`mcp__` 命名空间的 MCP tools 或再次委派。这里的“只读”默认是受控语义而不是 OS sandbox：可证明属于严格只读 allowlist 的 Bash 直接执行；其他 Bash 使用固定边界，在 interactive环境进入与主 Agent相同的审批流程并共享 allow-all、tool和精确 command会话授权缓存，在 headless环境直接 fail closed，且子 runtime不接收或判断父级 interaction mode。当父 run 声明 readonly 工具策略时，只读子运行继承父 run 的只读 bash 边界：生效沙箱可用且档位为 `read-only` 时任意 Bash 交内核沙箱边界直接执行且不进入审批（判定与执行包装同源）；沙箱未生效或档位不是 `read-only` 时仅严格只读 allowlist 内的命令执行、其余 fail-closed 拒绝。子 Agent origin只负责来源展示和迟到请求隔离；拒绝、Esc或反馈作为失败 tool result返回当前子 Agent。父 Esc的 abort signal继续传播到子 provider和可中断工具，并按父 turn取消语义收尾。

`worker` 的独立registry包含当前文件编辑工具、Bash、读取搜索、Web、Skill（scoped registry）、独立Todo、`ask_user_questions`和共享manager已初始化的MCP tools，但仍不含`run_subagent`。Worker在normal中复用主Agent普通风险分类和共享审批缓存/change recorder，在plan中拒绝文件写入与MCP，Bash按主Agent同一分层处理：继承父运行的沙箱收紧，生效只读沙箱下任意Bash交内核边界直接执行，未生效时仅严格只读allowlist，其余直接拒绝且不打开审批surface；headless默认deny，显式full-access只放行普通approval-required调用。问题通过带runId的专属callback进入共享`UserQuestionContext`，等待时活动为`waiting_question`，父turn取消会解析等待Promise并隔离迟到答案。自动审批只信任原始用户请求及结构校验后的同run问题答案，Worker委派任务始终位于不可信上下文分区。

子运行的 start、稳定 reasoning/assistant、内部工具 call/result 和终态作为本地 `subagent` role 增量持久化；token、spinner frame 和 provider-private reasoning 不落盘。最终报告由 assistant event供 rail重放、由外层 tool result供主 provider消费；completed record只保存状态与耗时，不再重复正文。该 role 会参与 resume/fork/undo 和 destructive replay，但统一从 provider converter、主 context构造、token估算、compaction摘要及 conversation reference 素材中排除；主 provider只能看到外层 `run_subagent` call/result及其最终结论。Renderer按runId恢复完整委派任务、带实际Agent名称的强调色标题和使用`subagentRail`专属主题色的最外层过程轨道，其余内部工具与思考内容统一使用`toolOutput`；只有start而没有终态的历史运行显示意外中断，不自动续跑。带`parallelSize`的并行子运行不进入主窗口rail投影：执行中由plural紧凑footer块（共享标题、按start顺序每run一行、`… +N more`折叠、ctrl+o提示）承担实时可见性，完成后外层`run_subagent` tool pair展开显示最终报告正文；无标记的单run完全保持既有rail流式与compact pair行为。Ctrl+O打开subagent会话窗口：destructive repaint按runId过滤主transcript投影单个run的完整loop内容，footer由run索引块直接贴状态行（运行中/总量计数、当前观看▸标记与上下折叠提示，不再渲染composer输入框），状态行展示切换序号、agent、任务、phase与耗时以及子运行实际使用的模型/effort（start record持久化该事实，旧记录缺省时回退主会话模型），打开时优先定位最近一个仍在活动的run，↑/↓始终在全部run（活跃与已完成）间循环切换，Esc或再次Ctrl+O关闭窗口且不中断父turn，授权/提问modal按既有优先级浮于窗口之上；窗口与`/btw`互斥。

自动审批只存在于交互式 app callback。风险分类仍先处理 safe、approval-required 和 plan/readonly rejected；单回合 resolver 先查询 `ToolApprovalContext` 的 allow-all、按工具和按 bash command 会话授权，再选择 manual 或 auto 路径。Auto `yes` 只映射为当前调用的 `allow_once`，不写会话缓存。Reviewer 使用严格 profile 解析、固定且带明确允许/拒绝标准的 system prompt、最近十条 user/assistant/contextual shell/tool call/tool result 文本和当前调用；它不装配工具，不加载 MCP、skills、memory 或项目指令，并通过 `reasoningEffort: none` 让各 provider 省略 reasoning/thinking 配置及 Codex encrypted reasoning。响应和 provider-private records 不写主 transcript。Headless `--once` 继续由 runtime 的 deny/full-access policy 直接决策，不创建 reviewer。

`runAssistantTurn`（`src/app/assistant-turn-runner.ts`）把 agent 回调翻译为 app 状态：提交边界先由 `AppContext` 比较当前 mode 与上一条模型可见 mode；发生 normal/plan 切换时，`AppContext` 把切换说明和用户请求写入 provider-facing `text`，同时用 `displayText` / `historyText` 保持 transcript 与 composer 只展示用户原文。该 transition 只写入切换后首条 user record，随 session、resume 和 compaction 持久化；同 mode 后续消息不重复注入。显式 slash skill 的固定模型成功解析后，runner 在 user record 后追加一条仅本地可见且可持久化的模型切换 `local_notice`；动态策略或陈旧 profile 回退不追加。随后 `onThinking` / `onToken` / `onReasoningUpdate` 驱动 spinner 与当前 pending 阶段。OpenAI Responses 在 `response.completed` 时发送唯一 reasoning complete；Chat/Anthropic 继续在各自协议完成边界发送 `complete`。正常 reasoning record 只由该 `complete` 回调创建，`onAssistantSegment`、`onToolCall` 和 `onComplete` 不再兜底完成 reasoning；只有失败或用户中断会把尚未完成的 reasoning 草稿保存为 partial record。app 组合根持有唯一约 100ms 的 activity timer，每个 tick 只刷新当前可见且仍在计时的 Main 或 BTW owner：assistant 正文使用 Markdown block 边界，reasoning 纯文本按当前 terminal width 提交最后一个仍可能增长的视觉行之前的内容，不等待 provider done。首个正文 token 到达时 runner 先推出当时已有的 reasoning 尾行并关闭其实时显示阶段，再开始正文 footer；之后迟到的 reasoning draft/complete 仍更新并提交完整 transcript 事实，但不再追加到正文之后。destructive replay 按完整最终 records 重建，因此恢复后可展示完整 reasoning。`onAssistantSegment` 把每次 provider run 当作独立 segment 落盘并重置正文显示进度，工具 continuation 的下一段从新进度开始。`onCompacted` 追加压缩提示，`onContextUsage` 更新 status line，`onToolCall` 暂存 pending preview，`onToolApprovalRequest` / `onUserQuestionRequest` 转交对应 context，`onToolResult` 成对追加 tool records，`onComplete` 提交最终 assistant record。中断收尾改由 `TurnContext.interruptActiveAssistantTurn` 消费 pending 调用成对补齐 interrupted result，`main.ts` 按 reasoning → partial → 补齐 pairs → notice 的顺序渲染。交互式 tool approval lifecycle hooks 由 runner 在 `ToolApprovalContext.requestManual` 的真实 modal 边界派发，因此 session cache 和 auto yes 不产生审批 hook，auto no 只在回退人工 surface 后产生；headless deny/full-access 仍由 runtime 直接派发审批 hook。CLI 默认通过显式注入共享配置 Context 的 `createAgentLoopRuntime(...)` 编排真实 agent lifecycle：runtime 每轮消费调用方固定或 Context 当前 snapshot、初始化对应 provider agent、维护 continuation 记录，并把每次流式 turn 委托给 provider agent；interaction mode 继续驱动 plan 工具风险分类，runtime suffix 只同步当前 open todo。provider adapter 在 stream 完成时尽量回传输入、缓存命中输入、缓存创建输入和输出 token；runtime 用最近一次 context usage 回调继续服务 `/context` 和 status line，同时把可用 provider usage 作为非敏感事件追加到 usage store，写入失败只进 debug 事件，不污染 transcript 或中断 assistant turn。lifecycle hooks 由 `main.ts` 装配一次并注入 runner/runtime；它们只观察 assistant turn、tool call 和 compaction 事件，hook 输出和失败不进入 renderer、transcript、session 或 provider request。`/hooks` 保存通过 `CommandHost.hooks` 更新用户配置后调用 dispatcher reload，reload 只影响后续 emit，已入队或正在运行的 hook job 继续使用入队时捕获的 entry 与 payload；synthetic test 走独立执行入口，不触发真实 lifecycle event，捕获的 stdout/stderr 只投影到当前 footer surface。provider adapter 只在 provider 边界转换 transcript，因此本地 `error`、`local_notice`、`compaction_notice` 与可见 `reasoning_summary` record 可持久化、可恢复，但不会发送给模型。
...
- MCP 只读放行：仅 `readOnlyHint: true` 的工具免审批直接执行；server 级 `never` 的写工具在只读运行（BTW、`/review`）中仍被拒绝且不打开审批 surface；工具缺失注解时保持原有审批
- 退出后通过 `/resume` 恢复上述 shell/tool transcript，确认 marker 仍可见且路径仍可回读

## Bash 沙箱

`run_bash_command` 默认经平台沙箱包装执行(macOS 用 `sandbox-exec`(Seatbelt),Linux 用 `bubblewrap`),定位是审批流之外的防御纵深,不是完整安全边界:

- `src/sandbox/` 提供 provider-neutral 抽象:`SandboxPolicy`(`off` / `read-only` / `workspace-write`、`network`、`extraWritablePaths`)与 `SandboxProvider`(把 `(shell -lc command)` 包装为平台 spawn argv);`resolveSandboxProvider` 按平台解析,darwin 返回 Seatbelt 实现,linux 返回 bubblewrap 实现,其他平台返回 null,后续接入其他系统沙箱只需新增 provider
- 两平台共享同一策略语义:文件读取全盘放行,写入限定在 realpath 归一化后的工作区、进程 TMPDIR、平台临时目录(macOS `/private/tmp`、Linux `/tmp`)、`~/.echo/agent-memory`、`/dev/null` 与用户追加目录;网络由 `tools.sandbox.network` 决定,`read-only` 档恒为禁网
- Seatbelt profile 由 `deny default` 加定向 allow 生成;Seatbelt 下 setuid/restricted 系统二进制(如 `ps`、`top`)无法 exec(报 `Operation not permitted`),属 Seatbelt 预期限制而非配置问题;`git`、`node`、`lsof` 等常规工具链不受影响
- Seatbelt 可用性探测同样分两级:先检查 `/usr/bin/sandbox-exec` 是否存在,再以最小 profile(`(version 1)(allow default)`)试运行探测当前环境能否实际应用 seatbelt(嵌套在另一个 seatbelt 沙箱内时内核会拒绝 apply);探针目标固定为 macOS 恒定存在的 `/usr/bin/true`,指向不存在的路径会让沙箱在整台机器上静默不可用;试运行结果缓存在 provider 实例内,失败时按无沙箱降级(`wrapCommand` 返回 null,命令不再逐条 exit 71),并区分「二进制缺失」与「试运行失败」两类降级原因
- bubblewrap argv 按「先全局只读、后定向放行」组织(bwrap 的文件系统选项按命令行顺序生效,`--ro-bind / /` 必须最先挂,否则会把后到的 `--dev`/`--proc` 盖成宿主只读视图,`/dev/null` 写不进):`--die-with-parent` 开头,禁网时加 `--unshare-net`,依次 `--ro-bind / /`、`--dev /dev`(最小设备集)、`--proc /proc`、`--tmpfs /tmp`、`--tmpfs /dev/shm`,再按档位追加工作区(严格 `--bind`)、`~/.echo/agent-memory` 与 `extraWritablePaths`(best-effort 预创建后 `--bind-try`)的可写 bind,尾部仍是 `<shell> -lc <command>`;进程 TMPDIR 不在 `/tmp` 下时补 bind,位于 `/tmp` 下由 tmpfs 覆盖
- bubblewrap 可用性探测分两级:先发现 bwrap 二进制(`/usr/bin/bwrap` 优先,再按 `PATH` 扫描),再试运行与真实命令同构的 mount 形态(`--ro-bind / / --dev /dev --proc /proc --tmpfs /tmp /bin/true`)并把结果缓存在 provider 实例内;二进制存在但试运行失败(嵌套容器无法挂 proc 等)同样按不可用降级
- Linux 发行版差异:Ubuntu 23.10+/24.04 的 AppArmor 会限制 unprivileged user namespaces,发行版 bubblewrap 包通常自带放行 profile,手动安装的 bwrap 可能需要用户放置 `/etc/apparmor.d/bwrap` 并 reload;容器内 seccomp 也会拒绝建立 user namespace——这些环境统一落入试运行失败的显式降级路径
- 不做 PID namespace 隔离(`--unshare-pid`),保持进程组终止语义最简单,后续按实际失败案例迭代收紧
- 运行级只读收紧:只读 workflow 可显式声明 `sandboxModeOverride: 'read-only'`,plan interaction mode 的 default 工具策略运行会自动派生同一收紧(显式声明优先);仅对非 `off` 配置生效(`off` 与 headless `full-access` 豁免优先级不变),把生效档位收紧为恒禁网的 `read-only`,并让执行包装、transient 注记、`/status`、委派子运行与 bash 边界判定共用同一份解析结果
- 默认档位为 `workspace-write` 且网络开启;`~/.echo/config.json` 的 `tools.sandbox` 可覆盖,非法档位或取值在配置解析期显式报错
- 执行语义不变:timeout、Esc 中断(进程组 kill)、输出截断与 offload 都作用在包装后的整个进程组上;headless `--full-access` 强制关闭沙箱,headless `deny` 下照常生效
- 用户 shell 模式、lifecycle hooks、MCP server 进程与 Node 侧内置工具不套沙箱;对 default 且非 plan 的运行,审批流(`tool-risk-classifier`)与沙箱正交,已批准命令在其沙箱边界内执行;只读运行与 plan 运行在生效只读沙箱下把 bash 主边界交给内核(文本白名单与人工审批退出),沙箱未生效时回退严格只读白名单——readonly 运行保持 fail-closed 拒绝,plan 运行拒绝白名单外命令且不打开 approval surface
- 生效只读沙箱内的 bash 命令无法写工作区,不构成不可追踪写入:change checkpoint 失效判定与执行包装同源(同一 provider 实例与缓存探测),只在命令不实际经只读沙箱时按文本白名单失效;该豁免对只读运行与 plan 运行同样生效
- 沙箱前端缺失、试运行失败或平台不支持时按无沙箱降级,`/status` 展示档位、网络、实现标识与降级原因(由 provider 实现给出),降级不静默;沙箱生效时在内置 transient 系统上下文追加一行边界说明,让模型对受限命令有预期
...
表格渲染拆在 `src/render/markdown-table.ts`，避免 Markdown renderer 变成单文件引擎。parser 只在连续 header + delimiter 被确认后生成 table block；streaming boundary scanner 与最终 parser 共享 fence/table 判定，并保留 draft 末块、table header candidate、未闭合 fence/table，以及 table 后仅有空白行的部分。只有后续非空 block 到达后 table 才成为稳定 source prefix，避免新增 row 改变已经焊入 scrollback 的列宽。table renderer 支持有/无外侧 pipe、escaped pipe、对齐、row cell count 归一化、中文宽字符 display width 和 cell wrap，使用无外框 Unicode 内部分隔线；终端过窄时降级为普通 pipe 文本投影。
...
| assistant pending | `◇` | thinking 时显示点阵 spinner；`streaming` 保留未稳定 Markdown 尾部，`reasoning_streaming` 保留最后一个仍可能增长的视觉行；activity tick 把更早内容逐步移入 terminal scrollback |
| assistant transcript | `◆` | 完成后仍追加包含完整 source 的 record；renderer 只补写尚未展示的正文并恰好追加一次 block suffix |
...
| `src/app/subagent-view-controller.ts` | `SubagentViewController` | Ctrl+O subagent 会话窗口：按 runId 过滤主 transcript 的只读全屏投影、↑/↓ 循环切换、Esc 关闭不中断父 turn、状态行与 draft 尾部投影 |
...
| `src/tools/tool-registry.ts` | `createDefaultToolRegistry`、`createToolRegistry` | 按配置装配本地工具目录；bash handler 按平台与实际生效的沙箱收紧包装，plan mode 不裁剪 provider-visible schema |
| `src/tools/tool-executor.ts` | `createToolExecutor`、`execute`、`parseArguments` | 统一执行工具 handler，处理查找、参数解析、异常归一化 |
| `src/tools/tool-risk-classifier.ts` | `classifyToolCallRisk`、`classifyReadonlyToolCall`、`parseBashCommand` | 执行前策略分类：安全执行、请求审批或按 mode 直接拒绝，覆盖 apply_patch、高风险 bash、MCP 只读注解边界与 MCP 资源读取（资源读取工具直接判安全，plan 与只读运行同样放行，并与其它只读调用共享 `parallel_read` 分类）；plan 与 readonly 分支接受 bash 沙箱生效标志，生效只读沙箱下 bash 交内核边界，未生效回退严格只读 allowlist；普通与只读运行的 MCP 分支都只放行 server 声明 `readOnlyHint` 的工具，其余进入审批或（只读运行）以专用文案 fail-closed |
