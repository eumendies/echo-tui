...
| command host | command handler 可用的受控 app 协议。app 组合根通过领域 command port factory 装配 transcript、clipboard、model、config、skills、mcp、memory、hooks、mode、goal、theme、context、status、usage、diff、undo、assistant、ui；runtime 把 session open/update/close/getActive 组合进同一个 host。composer 消费统一由 `submitComposer()` 完成，不暴露给 handler | `src/app/command/command-host.ts`、`src/app/command/*-command-port*.ts`、`src/app/command/command-runtime.ts` |
| tool approval context | agent 请求写文件、高风险 shell 或需授权 MCP 工具前的授权状态。会话缓存优先；`manual` 直接投影现有 `choice` surface，`auto` 在缓存未命中时由单回合 resolver 调用独立 reviewer。Reviewer 使用指定 profile、带信任分区的有界用户请求与动作投影、无工具 provider 和 `reasoningEffort: none` 判断，仅精确 `yes` 产生一次性允许，其余结果回退同一人工 surface。支持 Allow once、Allow tool/session、Allow command/session、Allow all/session、Deny 和 Tell model what to do；bash 只按完全相同 command 文本复用授权。主 Agent和子 Agent共享同一缓存与审批流程；子 Agent origin只用于 surface标题和迟到请求隔离，不改变授权选项或缓存命中 | `src/app/state/tool-approval-context.ts`、`src/app/tool-approval/`、`src/tools/tool-risk-classifier.ts` |
...
| goal controller | `/goal` 声明的会话级常驻目标：`GoalState` 经 `set_goal_state` 与 notice 同批进 session journal，`/clear` 清除、`/fork` 继承、resume 归一化为 `paused` 并重置运行时基线。每个完成的主会话回合后由独立评估器（严格引用 `goal.evaluationModelProfileId`，无工具 provider、`reasoningEffort: none`、30s deadline）按 yes/no + 理由协议判定；未达成且未达上限（默认 20 轮）时经独立入口发起自动续跑回合，`unavailable`、回合失败或中断一律暂停等待 `/goal resume`；用户输入通道（response lock、pending 单槽、command session）优先，被占用即让位 | `src/app/goal/goal-controller.ts`、`src/app/goal/evaluator.ts`、`src/app/goal/evaluation-projection.ts`、`src/commands/goal-command-handler.ts` |
| status line | 普通输入 footer 底部的一行 segmented 状态。左侧展示当前 session 内存模型、有效推理等级和项目名，右侧展示最近一次真实 provider context usage 与输入/工作模式；普通 redraw/render path 只读 `ModelContext` 缓存。`/model`、`/effort` 和 `Ctrl+T` 立即刷新该缓存并清除旧 usage；sidecar 同步只是静默恢复优化，失败不改变 status line 或阻断请求。`/config` 或 watcher 只刷新全局 catalog/default，当前 profile 仍有效时保持 session 选择，profile 被删除时回退全局默认并清理旧 usage。显式 slash skill 在 active assistant turn 内按字段覆盖 session 展示为 `<model> (SKILL override)`，完成、失败或中断后恢复 session 状态；无效 skill profile 回退 session model。存在 goal 时右侧额外展示 goal 段：active 展示 `N/M` 轮次进度，paused 展示暂停标记，评估请求进行中显示评估中指示；无 goal 时隐藏。command/approval/user-question/file-picker surface 会替换普通输入区域 | `src/render/footer.ts`、`src/types/render.ts`、`src/app/state/model-context.ts`、`src/app/state/turn-context.ts` |
| transcript store | 按 cwd hash 分区的本地 append-only JSONL session journal。默认根目录是 `~/.echo/echo_tui/`；每个 session 以 `session_start` 首行建立身份，随后逐行追加 records、compaction、todo、goal 状态和 `/undo` / `/diff` 共用的 change history 操作。子 Agent 的 start、稳定文本、内部工具和终态继续使用普通 `append_records` 增量落盘；预览把连续同 run 过程压成单条摘要。同目录 `{session-id}.settings.json` sidecar 原子覆盖当前 model profile ID 和可选 effort override，不保存 profile 定义或历史值；孤立 sidecar 不进入 session 枚举。journal 重放校验 envelope、record role 和下游依赖的关键身份字段，包括 subagent run/parent call identity 与事件结构，但保留未知扩展字段；display metadata 等可选 payload 由消费端自行校验。重放忽略末尾未完成写入，真正恢复 session 时会原子移除无效尾部后再允许续写，但拒绝中间损坏；不保存 composer、pending、command session 或输入历史 | `src/persistence/transcript-store.ts`、`src/persistence/transcript-journal.ts`、`src/persistence/session-model-settings-store.ts` |
...
| tool message rendering | app 可见层把未完成 tool call 显示为 footer pending preview；需要用户参与的工具先进入 approval 或 user-question surface；tool result 到达后再追加具备必填 call id/name/status 和 discriminated `details.kind` 的 `tool_call` / `tool_result` records。顶层 renderer 负责路由和通用 fallback，子目录分别承载 bash、apply_patch、use_skill 及共享宽度/前缀/rail 行逻辑。Subagent renderer 按 runId 生成外层 rail，在扣除宽度后复用内部工具行级 renderer；完整委派任务不受普通工具显示行数预算截断，实际Agent标题、任务文本和最外层marker/rail统一使用专属`subagentRail`主题色，内部工具标题、状态、prefix、正文以及reasoning/assistant/阶段文本全部映射到`toolOutput`。每个稳定工作事件及transient活动前插入不断开的外层rail空行。实时call在footer中保持瞬时投影，result到达后成对写入历史区；稳定区与footer共用rail几何，活动草稿由统一timer刷新。恢复时缺少终态的run显示意外中断；外层pair保留Explorer/Worker专属成功文案，自定义合法名称显示通用completed/failed身份。rail、footer详情、审批和提问标题均通过共享名称校验，自定义名称保留目录原名，损坏或含控制字符身份回退为Subagent。`use_skill` 成功结果只投影为简洁使用摘要，agent memory 脚本按普通 bash rail 展示，旧 memory tool records 按通用 fallback 展示 | `src/app/state/turn-context.ts`、`src/render/tool-message-renderer.ts`、`src/render/subagent-renderer.ts`、`src/render/tool-message-renderers/` |
...
| `agents` | 为 `/agents` 捕获当前模型目录并扫描 Built-in/User/Project 物理定义，提供严格校验、原子 CRUD、内置 model/effort override 与冲突结果；handler 不直接访问文件系统或活动 Subagent catalog |
...
| `goal` | 为 `/goal` 提供会话级目标的读取与 set/pause/resume/clear 端口；回合完成后评估并自动续跑，评估模型未配置或引用失效时拒绝设置与恢复 |
...
主 Agent 的 `run_subagent` 是普通 `ToolHandler`，通过 `ToolRegistry` / `ToolExecutor` 和 run-scoped `SubagentToolPort` 同步执行。内置定义目录是各子 Agent name、description、prompt、本地工具集合、MCP可见性和执行策略的唯一来源；handler通过Port列出的name/description动态生成必填`agent` enum及参数目录，顶层工具description只保留通用委派语义。调用时Port按名称取得完整定义，并把定义交给factory创建新的专用subagent runtime；`SubagentLoopInput` / `SubagentLoopCallbacks`只交付任务、身份、取消、mode和活动协议，不伪造主`AgentSessionInput`，也不递归复用父`RunAgent`。外层handler使用`pair_after_execute` commit mode：Port把子callback翻译出的稳定`subagent` records先提交到父runtime record region和主session journal，子运行结束后才成对提交外层`run_subagent` call/result。目录包含只读调查`explorer`和通用执行`worker`；二者都不继承父transcript、Todo、compaction或journal，只继承父run已捕获的配置revision、cwd、适用AGENTS指令、memory和skill catalog，并受单层嵌套和每父run四次委派限制。

`explorer` 的 provider-visible registry 和执行 registry 都只包含 `read_files`、`glob`、`grep`、`run_bash_command`、`web_fetch`、`web_search`、`use_skill`，不包含写文件、Todo、提问、MCP 或再次委派。这里的“只读”是受控语义而不是 OS sandbox：可证明属于严格只读 allowlist 的 Bash 直接执行；其他 Bash 使用固定边界，在 interactive环境进入与主 Agent相同的审批流程并共享 allow-all、tool和精确 command会话授权缓存，在 headless环境直接 fail closed，且子 runtime不接收或判断父级 interaction mode。子 Agent origin只负责来源展示和迟到请求隔离；拒绝、Esc或反馈作为失败 tool result返回当前子 Agent。父 Esc的 abort signal继续传播到子 provider和可中断工具，并按父 turn取消语义收尾。

`worker` 的独立registry包含当前文件编辑工具、Bash、读取搜索、Web、Skill、独立Todo、`ask_user_questions`和共享manager已初始化的MCP tools，但仍不含`run_subagent`。Worker在normal中复用主Agent普通风险分类和共享审批缓存/change recorder，在plan中拒绝文件写入、非只读Bash与MCP；headless默认deny，显式full-access只放行普通approval-required调用。问题通过带runId的专属callback进入共享`UserQuestionContext`，等待时活动为`waiting_question`，父turn取消会解析等待Promise并隔离迟到答案。自动审批只信任原始用户请求及结构校验后的同run问题答案，Worker委派任务始终位于不可信上下文分区。

子运行的 start、稳定 reasoning/assistant、内部工具 call/result 和终态作为本地 `subagent` role 增量持久化；token、spinner frame 和 provider-private reasoning 不落盘。最终报告由 assistant event供 rail重放、由外层 tool result供主 provider消费；completed record只保存状态与耗时，不再重复正文。该 role 会参与 resume/fork/undo 和 destructive replay，但统一从 provider converter、主 context构造、token估算、compaction摘要及 conversation reference 素材中排除；主 provider只能看到外层 `run_subagent` call/result及其最终结论。Renderer按runId恢复完整委派任务、带实际Agent名称的强调色标题和使用`subagentRail`专属主题色的最外层过程轨道，其余内部工具与思考内容统一使用`toolOutput`；只有start而没有终态的历史运行显示意外中断，不自动续跑。
...
| `agents` | `/agents` Subagent 管理面板 | 渲染来源 Tab、有效/覆盖/无效状态、详情、显式动作行、字段与工具表单、默认取消确认和反馈；name/description/instructions 编辑态显示真实光标 |
...
| `status` | `/status` 运行状态与 Codex 配额面板 | 渲染本地运行状态；存在 goal 时追加只读摘要行（状态、条件摘要与 `N/M` 轮次进度）。以进度条展示 Codex OAuth 5 小时/每周配额、百分比和重置时间，隐藏光标 |
...
| `src/commands/goal-command-handler.ts` | `GoalCommandHandler`、`createGoalStatusSurface` | `/goal` 状态卡与 set/pause/resume/clear（stop/off/reset/cancel 为清除别名） |
...
| `src/app/goal/goal-controller.ts` | `createGoalController`、`createGoalContinuationTurn` | 目标生命周期与自动续跑循环；revision 隔离迟到评估与回合结果，评估请求期间暴露评估活动 |
| `src/app/goal/evaluator.ts` | `createGoalEvaluator`、`parseGoalEvaluationResponse` | 独立 goal 评估器：yes/no + 理由协议、30s 组合超时、失败关闭为 unavailable |
| `src/app/goal/evaluation-projection.ts` | `createGoalEvidenceProjection` | 评估证据的有界投影（条数、总字符与单条字符上限） |
| `src/app/command/goal-command-port.ts` | `createGoalCommandPort`、`resolveGoalEvaluationModelProfileId` | 在组合根装配 goal 控制器并严格解析评估模型 profile |
| `src/app/deadline-scope.ts` | `createDeadlineAbortScope` | deadline 与 parent abort 组合的取消作用域，供 tool approval reviewer 与 goal evaluator 复用 |
...
| `src/app/state/transcript-context.ts` | `TranscriptContext` | 持有 transcript records、compaction、todo、goal 状态、持久化 change history 和 session 指针；负责 journal 持久化/恢复、compaction 应用和 resume metadata |
...
| `src/app/state/render-context.ts` | `RenderContext` | 持有 terminal、previous size 和当前 theme，结合 composer、turn、mode、model、context usage 与 goal 状态输入派生 banner/footer render state |
...
| `src/agent/subagent/catalog.ts`、`manifest.ts`、`name.ts`、`settings.ts` | `loadSubagentCatalog`、`parseCustomSubagentManifest`、`serializeCustomSubagentManifest`、名称 formatter | 合并内置、用户和项目定义，严格解析模型/effort与能力策略并冻结目录，加载内置 override sidecar，输出有界 debug 诊断，并为所有 App/render 身份提供控制字符安全回退 |
| `src/app/command/agents-command-port.ts`、`src/commands/agents-command-handler.ts`、`src/render/footer/agents-surface.ts` | `createAgentsCommandPort`、`AgentsCommandHandler`、`renderAgentsSurface` | 组合 `/agents` 受控文件端口、Enter 驱动的分层 CRUD/确认 controller 与响应终端宽高的专属 footer card |
