...
- Seatbelt 可用性探测同样分两级:先检查 `/usr/bin/sandbox-exec` 是否存在,再以最小 profile(`(version 1)(allow default)`)试运行探测当前环境能否实际应用 seatbelt(嵌套在另一个 seatbelt 沙箱内时内核会拒绝 apply);试运行结果缓存在 provider 实例内,失败时按无沙箱降级(`wrapCommand` 返回 null,命令不再逐条 exit 71),并区分「二进制缺失」与「试运行失败」两类降级原因
...
- 运行级只读收紧:只读 workflow 可声明 `sandboxModeOverride: 'read-only'`;仅对非 `off` 配置生效(`off` 与 headless `full-access` 豁免优先级不变),把生效档位收紧为恒禁网的 `read-only`,并让执行包装、transient 注记、`/status` 与只读 bash 边界判定共用同一份解析结果
...
- 用户 shell 模式、lifecycle hooks、MCP server 进程与 Node 侧内置工具不套沙箱;对 default 运行,审批流(`tool-risk-classifier`)与沙箱正交,已批准命令在其沙箱边界内执行;只读运行在生效只读沙箱下把 bash 主边界交给内核(文本白名单与人工审批退出),沙箱未生效时回退严格只读白名单
