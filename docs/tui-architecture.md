...
| tool approval context | agent 请求写文件、高风险 shell 或需授权 MCP 工具前的授权状态。会话缓存优先；`manual` 直接投影现有 `choice` surface，`auto` 在缓存未命中时由单回合 resolver 调用独立 reviewer。Reviewer 使用指定 profile、最近十条文本上下文、无工具 provider 和 `reasoningEffort: none` 判断，仅精确 `yes` 产生一次性允许，其余结果回退同一人工 surface。支持 Allow once、Allow tool/session、Allow command/session、Allow all/session、Deny 和 Tell model what to do；bash 只按完全相同 command 文本复用授权 | `src/app/state/tool-approval-context.ts`、`src/app/tool-approval-resolver.ts`、`src/tools/tool-risk-classifier.ts` |
...
| agent loop runtime | provider-neutral 的真实 agent 编排层。assistant turn 启动时捕获一个配置 snapshot，再从同一 revision 一次解析 provider、reasoning、tool registry、context window、压缩阈值、指令文件名和 skill catalog 预算；整个 provider streaming、compaction 与 tool continuation 不切换 revision。用户 memory 仍按每次 provider request 的独立生命周期读取。系统 prompt、指令和运行时 suffix 不写入 transcript；provider usage 回传 app 并写入 usage store，tool call 与 compaction 事实事件旁路派发给 lifecycle hooks | `src/agent/agent-loop-runtime.ts`、`src/agent/context/system-prompt.ts` |
