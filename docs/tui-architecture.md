...
| `src/agent/subagent/catalog.ts`、`manifest.ts`、`name.ts` | `loadSubagentCatalog`、`parseCustomSubagentManifest`、名称 formatter | 合并内置、用户和项目定义，严格解析并冻结能力目录，输出有界 debug 诊断，并为所有 App/render 身份提供控制字符安全回退 |
