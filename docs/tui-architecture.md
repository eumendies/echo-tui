...
| agent loop runtime | provider-neutral 的真实 agent 编排层位于同一 `loop-runtime` 包，但主 Agent 与子 Agent使用独立业务入口。主 runtime 在 assistant turn 启动时捕获配置 snapshot，再从同一 revision 解析 provider、reasoning、完整 tool registry、context window、压缩阈值、指令文件名和 skill catalog 预算，并负责 Todo、用户提问、MCP、普通审批、主 transcript callback和 `run_subagent` Port。每次已接受的委派都由 factory 创建新的 subagent runtime；子入口拥有自己的 continuation loop、只读工具执行、人工 Bash升级、hook和专属 callback协议，复用父 revision、cwd、项目指令、memory和 skills快照，但只从委派任务开始并固定只读 allowlist、MCP禁用和专属 prompt，不设置专属 provider turn上限。父子不共享业务 loop、runtime实例、provider continuation、record region、compaction或 registry；只共享 provider context、usage调试投影等无角色纯函数和进程级服务 | `src/agent/loop-runtime/agent-loop-runtime.ts`、`src/agent/loop-runtime/subagent-loop-runtime.ts`、`src/agent/loop-runtime/shared.ts`、`src/agent/subagent/` |
...
| tool message rendering | app 可见层把未完成 tool call 显示为 footer pending preview；需要用户参与的工具先进入 approval 或 user-question surface；tool result 到达后再追加具备必填 call id/name/status 和 discriminated `details.kind` 的 `tool_call` / `tool_result` records。顶层 renderer 负责路由和通用 fallback，子目录分别承载 bash、apply_patch、use_skill 及共享宽度/前缀/rail 行逻辑。Subagent renderer 按 runId 生成外层 rail，在扣除宽度后复用内部工具行级 renderer；Explorer 标题和最外层 marker/rail 使用 `tool` 强调色，内部工具标题、状态、prefix、正文以及 reasoning/assistant/阶段文本全部映射到 `toolOutput`。每个稳定工作事件及 transient 活动前插入不断开的外层 rail 空行。实时 call 在 footer 中保持瞬时投影，result 到达后成对写入历史区；稳定区与 footer 共用 rail 几何，活动草稿由统一 timer 刷新。恢复时缺少终态的 run 显示意外中断，外层 pair 只显示紧凑返回状态。`use_skill` 成功结果只投影为简洁使用摘要，agent memory 脚本按普通 bash rail 展示，旧 memory tool records 按通用 fallback 展示 | `src/app/state/turn-context.ts`、`src/render/tool-message-renderer.ts`、`src/render/subagent-renderer.ts`、`src/render/tool-message-renderers/` |
...
主 Agent 的 `run_subagent` 是普通 `ToolHandler`，通过 `ToolRegistry` / `ToolExecutor` 和 run-scoped `SubagentToolPort` 同步执行。内置定义目录是各子 Agent name、description、prompt和工具 allowlist的唯一来源；handler通过 Port列出的 name/description动态生成必填 `agent` enum及参数目录，顶层工具 description只保留通用委派语义。调用时 Port按名称取得完整定义，并把定义交给 factory创建新的专用 subagent runtime；`SubagentLoopInput` / `SubagentLoopCallbacks`只交付任务、身份、取消和活动协议，不伪造主 `AgentSessionInput`，也不递归复用父 `RunAgent`。外层 handler使用 `pair_after_execute` commit mode：Port把子 callback翻译出的稳定 `subagent` records先提交到父 runtime record region和主 session journal，子运行结束后才成对提交外层 `run_subagent` call/result；进程若中途退出，journal可以保留已发生的子过程，但不会留下孤立的外层 call。第一版目录只包含 `explorer`；它不继承父 transcript、todo、compaction或 journal，只继承父 run已捕获的配置 revision、cwd、适用 AGENTS指令、memory和 skill catalog，并受单层嵌套和每父 run四次委派限制；provider continuation不设置子 Agent专属轮数上限。
...
| `src/agent/loop-runtime/subagent-loop-runtime.ts` | `createSubagentLoopRuntime` | Explorer专用 runtime：隔离任务、裁剪 registry、只读/人工升级工具执行、独立 continuation、hook和专属 callbacks |
