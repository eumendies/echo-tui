...
| MCP manager | 启动期从共享配置 snapshot 初始化 enabled MCP servers，单 server 失败只记录脱敏诊断；以 `mcp__<server>__<tool>` 命名空间暴露工具、按名调用、支持重载与关闭。对声明 `resources` capability 的 server，启动期同时按分页与条目上限拉取并缓存资源与模板目录，暴露 `listResources`/`listResourceTemplates`/`readResource`；资源端点失败只降级为空目录并记录脱敏诊断，不影响工具能力。没有 server 级审批开关：审批边界只由工具 `annotations.readOnlyHint` 归一出的事实决定——声明只读的工具免审批，hint 缺失或为 false 时一律进入审批；同一事实也提供只读运行的 MCP 准入判定。`/mcp` 保存后消费 writer 已安装的 revision 并显式 reload 一次；外部 watcher 变化只更新 Context，不隐式关闭或连接 MCP 进程 | `src/mcp/manager.ts`、`src/mcp/client.ts`、`src/config/mcp-config.ts` |
