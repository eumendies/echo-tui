...
系统 SHALL 渲染底部 footer。footer 由可选 pending preview 和当前输入 surface 组成：普通输入态的 surface 为 1 到 N 行 composer 和固定 1 行 hint；help overlay 态的 surface 为覆盖在 composer 区域的帮助内容和退出提示。assistant streaming pending preview SHALL 使用按 terminal rows 动态预算的 Markdown-aware 尾部预览，避免长输出时 footer 高度无限增长；当 draft 包含有效 table 时，该预览 SHALL 使用 table-aware projection。
...
系统 SHALL 使用轻量符号和克制颜色区分 transcript 中的用户消息、assistant 消息、本地 error 消息和 pending assistant，而不是在 transcript 中显示 `user:`、`assistant:` 或 `error:` 文本标签。用户消息的整行背景 SHALL 在渲染投影阶段按当前终端宽度计算。assistant 消息内容 SHALL 支持 Markdown-aware terminal projection，包括 table-aware projection；role 视觉标记 SHALL 继续由 transcript renderer 控制。
