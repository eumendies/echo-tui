...
#### Scenario: agent 和 persistence 迁移保持边界不变
- **WHEN** agent 与 persistence 模块迁移为 TypeScript
- **THEN** `src/agent/openai-agent.ts` SHALL 继续承载真实 LLM adapter，`src/agent/llm-config.ts` SHALL 继续承载用户级配置读取与校验，`src/agent/fake-agent.ts` SHALL 继续作为测试注入和显式开发 fixture
- **THEN** `src/persistence/transcript-store.ts` SHALL 继续承载本地 transcript session 存储、读取、列表派生和 atomic write
- **THEN** 迁移 SHALL NOT 让 app 层直接读取用户配置文件、直接调用 OpenAI SDK、直接操作 session JSON 文件或绕过 transcript store

#### Scenario: agent 和 persistence 迁移保持运行行为
- **WHEN** agent 与 persistence 模块迁移为 TypeScript
- **THEN** 真实 adapter lifecycle、fake agent callbacks、配置错误脱敏、cwd hash 分区、session JSON schema、metadata 派生和 atomic write SHALL 保持现有行为
- **THEN** slash 命令、普通提交、`/resume` 恢复和 `/clear` detach session 语义 SHALL 不因迁移发生用户可见变化
