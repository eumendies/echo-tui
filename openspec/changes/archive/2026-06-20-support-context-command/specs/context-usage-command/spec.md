...
系统 SHALL 提供 `/context` slash command，用于展示最近一次真实 provider request 的 context usage 详情。详情 SHALL 包含 used tokens、context window、窗口占用百分比，以及按 System prompt、Skills、Tools、Messages、Reasoning 分类的 token breakdown。
...
- **AND** surface SHALL 显示 System prompt、Skills、Tools、Messages、Reasoning 的分类占用
...
#### Scenario: system prompt 和 skill catalog 分别计入 System prompt 与 Skills
- **WHEN** agent loop 为 provider request 注入内置 system prompt
- **THEN** 系统 SHALL 将 skill catalog 以外的 system prompt 估算 tokens 归入 System prompt 分类
