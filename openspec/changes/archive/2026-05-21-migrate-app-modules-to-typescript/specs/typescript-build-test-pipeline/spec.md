...
- **THEN** 系统 SHALL 支持 `src/agent/fake-agent`、`src/agent/llm-config`、`src/agent/openai-agent` 和 `src/persistence/transcript-store` 使用 `.ts`
...
- **WHEN** `src/agent` 或 `src/persistence` 中的运行源码模块使用 TypeScript
- **THEN** LLM config 读取与校验、错误脱敏、SDK stream 归一化、fake agent 行为、cwd hash 分区、session JSON schema 和 atomic write 语义 SHALL 稳定
