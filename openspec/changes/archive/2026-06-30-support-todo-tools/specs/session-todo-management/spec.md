...
系统 SHALL 为每个 transcript session 维护结构化 `todoState`。`todoState` SHALL 随 session 保存和加载，不得作为普通 transcript record 保存，也不得参与 context compaction 边界计算。

#### Scenario: 保存包含 todoState 的 session
- **WHEN** 当前 session 存在未完成 todo
- **AND** app 持久化当前 session
- **THEN** session JSON SHALL 在顶层保存 `todoState`
- **AND** `todoState` SHALL 包含当前 todo items 和更新时间

#### Scenario: 恢复 session todoState
- **WHEN** 用户通过 `/resume` 加载包含 `todoState` 的 session
- **THEN** app SHALL 恢复该 session 的 todo 状态
- **AND** 下一次 provider 请求 SHALL 能看到恢复后的未完成 todo

#### Scenario: 旧 session 缺少 todoState
- **WHEN** app 加载旧 session 且 session JSON 不包含 `todoState`
...
系统 SHALL 提供 provider-visible `create_todos` 工具，用于创建当前会话的 todo list。调用该工具 SHALL 用新列表覆盖旧 `todoState.items`，旧的已完成或未完成 todo 不再作为当前运行时 todo 状态保留。
...
- **AND** tool result SHALL 返回创建后的 todo ids 和当前未完成 todo 列表
...
系统 SHALL 提供 provider-visible `complete_todo` 工具，用于将当前 `todoState` 中的一个或多个 todo 标记为 completed。该工具 SHALL 只修改会话级 todo 状态，不得修改工作区文件。
...
- **AND** tool result SHALL 返回已完成 id 和剩余未完成 todo 列表
...
系统 SHALL 为 `create_todos` 和 `complete_todo` 提供专属 tool message renderer。该 renderer SHALL 以 todo 列表形式展示工具调用后的当前状态，不得只展示原始 JSON。

#### Scenario: 渲染 create_todos 结果
- **WHEN** transcript 包含 `create_todos` 的 tool_result
- **AND** tool_result 文本包含可解析的 todo 状态
- **THEN** renderer SHALL 显示创建后的 todo 列表
- **AND** renderer SHALL 将 open todo 显示为未完成状态

#### Scenario: 渲染 complete_todo 结果
- **WHEN** transcript 包含 `complete_todo` 的 tool_result
- **AND** tool_result 文本包含 completed todo 和 remaining open todo
- **THEN** renderer SHALL 显示已完成和未完成 todo
- **AND** completed todo SHALL 显示勾选符号和删除线文本

#### Scenario: 强调第一项未完成 todo
- **WHEN** todo tool result 中存在一个或多个 open todo
- **THEN** renderer SHALL 用颜色或等价主题强调标记第一个 open todo
- **AND** 其他 open todo SHALL 保持普通未完成标记

#### Scenario: 全部完成的渲染
- **WHEN** todo tool result 中所有 todo 均为 completed
- **THEN** renderer SHALL 显示每个 completed todo 的勾选符号和删除线文本
- **AND** renderer SHALL NOT 显示未完成强调项
