...
- **WHEN** 当前生效模型 profile 引用的 provider preset 解析为 `agentType: "openai-chat"` 或 `agentType: "anthropic"`，且模型 profile 配置了 `reasoning.effort` 或 `reasoning.summary`
...
### Requirement: provider profile 直接配置 agentType
**Reason**: `/config` 需要隐藏协议实现细节，并通过 provider preset catalog 中心化管理 `agentType`、固定 Base URL 和 headers。

**Migration**: 用户应通过 `/config` 重新保存 provider-backed preset 配置，或手动把 provider profile 改为包含 `preset`、`apiKey`、可选 `baseURL` 的新结构。
