...
系统 SHALL 提供本地工具 `read_files`，用于按已知路径读取一个或多个本地文件。该工具 SHALL 接收 JSON object 参数 `{ "files": Array<{ "path": string, "offset"?: number, "limit"?: number }> }`，并 SHALL 返回可回传模型的 bounded tool execution result。`offset` 与 `limit` SHALL 仅对文本文件读取生效；图片 reader SHALL 忽略这些字段而不把字段本身视为错误。文本文件结果 SHALL 在内容块中包含真实的 1-based 文件行号，并 SHALL 使用 metadata 标明返回片段的起止文件行号。受支持图片文件结果 SHALL 包含人类可读 metadata，并 SHALL 通过 provider-neutral 图片附件把图片内容提供给后续模型请求；handler SHALL NOT 把二进制内容或 base64 原样写入 result 文本。
...
- **THEN** 任一文件失败时整体 result SHALL 标记 `ok: false`，但成功文件的文本内容和图片附件 SHALL 仍保留在 result 中

#### Scenario: 暂不支持的非文本媒体类型返回明确 metadata
- **WHEN** `read_files` 收到 PDF、BMP 或其他暂不支持的非文本文件路径
...
#### Scenario: 限制读取规模、图片规模和输出规模
- **WHEN** files 数量、单文件本次返回文本内容 bytes、单张图片 bytes 或总输出 bytes 超过内置安全上限
- **THEN** handler SHALL 返回 `ok: false` 或在安全边界内截断输出
- **THEN** result 文本 SHALL 明确说明失败或截断原因
- **THEN** result SHALL 在发生文本输出截断时标记 `truncated: true`
