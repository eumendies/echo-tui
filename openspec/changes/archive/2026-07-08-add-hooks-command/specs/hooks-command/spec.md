...
- **WHEN** hook synthetic test 完成
- **THEN** hooks command surface SHALL 展示测试 exit code 或 timeout 状态
- **THEN** hooks command surface SHALL 展示测试耗时
- **THEN** hooks command surface SHALL 可展示截断后的 stdout 和 stderr
- **THEN** hooks command surface SHALL 标明真实 lifecycle hook 运行时输出仍会被忽略
