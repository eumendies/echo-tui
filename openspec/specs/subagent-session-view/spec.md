# subagent-session-view Specification

## Purpose

定义通过 Ctrl+O 打开的 subagent 会话窗口：以 destructive repaint 全屏接管查看子 Agent 运行的完整 loop 过程，支持并行运行切换、与更高优先级 surface 共存、Esc 返回主会话，并保证窗口内容与恢复投影一致。
