...
默认单轮模式 SHALL 允许不需要审批的安全工具继续执行。对于需要用户 approval 的工具，系统 SHALL 立即生成可回传模型的拒绝 tool result；对于 `ask_user_questions`，系统 SHALL 立即生成取消或失败 tool result。单轮模式 SHALL NOT 等待不存在的 TUI 输入。

#### Scenario: 安全工具继续执行
- **WHEN** 默认单轮 agent 请求安全的只读工具
- **THEN** 系统 SHALL 使用现有 tool executor 执行该工具
- **AND** 工具结果 SHALL 继续回传给 agent loop

#### Scenario: approval-required 工具默认拒绝
- **WHEN** 默认单轮 agent 请求 `apply_patch`、高风险 bash、memory mutation 或未信任 MCP tool
...
当且仅当用户使用 `echo-tui --once --full-access <prompt...>` 时，系统 SHALL 自动允许当前单轮中被风险分类为 approval-required 的已注册工具。该选项 SHALL 不启用未配置的工具、不改变 plan mode 规则或普通 TUI approval 行为，并 SHALL 在帮助或错误提示中说明其可能修改工作区或系统状态。

#### Scenario: full-access 自动允许高风险工具
- **WHEN** 用户使用 `--once --full-access` 且 agent 请求高风险 bash、`apply_patch`、memory mutation 或未信任 MCP tool
- **THEN** 系统 SHALL 不打开 approval surface
- **AND** 系统 SHALL 直接执行已注册工具并把结果回传给 agent loop

#### Scenario: full-access 不改变普通 TUI
- **WHEN** 用户不使用 `--once --full-access` 而在普通 TUI 中运行 agent
