...
系统 SHALL 暂存调节模式中的选择，直到用户确认。Enter SHALL 在一次配置事务中应用全局 selected model 和目标 profile effort；Esc 或再次按 `Ctrl+T` SHALL 取消全部暂存选择。

#### Scenario: Enter 原子应用选择
- **WHEN** 用户在调节模式中选定 model 与 effort 并按下 Enter
- **THEN** 系统 SHALL 在一次用户配置更新中写入 `llm.selectedModel` 和目标 profile 的 `reasoning.effort`
- **THEN** 写入成功后系统 SHALL 刷新模型状态缓存并退出调节模式
- **THEN** 后续 provider turn 和普通 status line SHALL 使用已应用的全局选择

#### Scenario: 模型变化清理旧 context usage
- **WHEN** Enter 成功应用了不同于进入调节模式时的 selected model
- **THEN** 系统 SHALL 清空旧 model 对应的 transient context usage
- **THEN** 新的真实 provider usage 到达前 status line SHALL NOT 显示旧 usage

#### Scenario: Esc 取消暂存选择
- **WHEN** 用户调整一个或多个候选后按下 Esc
- **THEN** 系统 SHALL 退出调节模式并丢弃暂存选择
- **THEN** 用户配置、模型状态缓存、composer 草稿和 transcript SHALL 保持不变
...
#### Scenario: 配置写入失败
- **WHEN** 用户按 Enter 应用选择但配置校验或写入失败
- **THEN** 系统 SHALL NOT 把未持久化的选择写入模型状态缓存
- **THEN** 系统 SHALL 保持调节模式和 composer 草稿
- **THEN** status line SHALL 显示脱敏的失败摘要，并允许用户重试或取消
...
composer model/effort 调节状态 SHALL 只存在于交互式 TUI，并 SHALL NOT 改变 headless、provider、transcript 或 session 持久化协议。
...
#### Scenario: 调节状态不进入持久化记录
- **WHEN** transcript session 或 input history 被保存
- **THEN** 活动字段、暂存 model、暂存 effort 和调节错误 SHALL NOT 写入 transcript record 或 session journal
