 ...
 */
function estimateContextUsageSegments(records: TranscriptRecord[], toolDefinitions: ToolDefinition[] = [], skillCatalogTokens = 0): EstimatedContextUsageSegment[] {
  ...
  for (const record of records) {
    ...
    if (category === 'system' && !countedSkills && skillCatalogTokens > 0) {
      addTokens(totals, 'system', Math.max(0, tokens - skillCatalogTokens));
