...
export function createAppRenderer(output: NodeJS.WriteStream = process.stdout): AppRenderer {
   ...
   *
   */
  function renderInitial({ bannerContext, composer, conversationReference, pendingMessage, commandSurface, slashSuggestions, pending, working, theme, renderPreferences, statusLine, rows, width }: RenderInitialOptions): void {
    output.write(renderBanner(bannerContext, theme));
    footer.render({ composer, conversationReference, pendingMessage, commandSurface, slashSuggestions, pending, working, theme, renderPreferences, statusLine, rows, width });
  }
   ...
   *
   */
  function appendRecord({ record, composer, conversationReference, pendingMessage, commandSurface, slashSuggestions, pending, working, theme, renderPreferences, statusLine, rows, width }: AppendRecordOptions): void {
    appendRecords({ records: [record], composer, conversationReference, pendingMessage, commandSurface, slashSuggestions, pending, working, theme, renderPreferences, statusLine, rows, width });
  }

  /**
   * transcript 成组新增事实内容时，一次性清 footer、append blocks、再重绘 footer。
   */
  function appendRecords({ records, composer, conversationReference, pendingMessage, commandSurface, slashSuggestions, pending, working, theme, renderPreferences, statusLine, rows, width }: AppendRecordsOptions): void {
   ...
   *
   */
  function renderDestructive({ bannerContext, records, composer, conversationReference, pendingMessage, commandSurface, slashSuggestions, pending, working, theme, renderPreferences, statusLine, rows, width }: RenderDestructiveOptions): void {
    const footerLayout = renderFooterLayout({ composer, conversationReference, pendingMessage, commandSurface, slashSuggestions, pending, working, theme, renderPreferences, statusLine, rows, width });
