...
- Stay read-only: this run enforces a read-only boundary at runtime. Write tools and MCP tools are rejected before execution; shell commands either run inside a read-only sandbox (no workspace writes, no network access) or, when no sandbox is available, must stay within the strict read-only inspection allowlist. Read-only subagents remain available for isolated investigation. Do not attempt output redirection, installs, builds, or test runs.
...
- Verify by reading the code path, tests, types, or a strictly read-only inspection command; do not attempt test or build runs, which the read-only boundary blocks. If a command fails and you cannot tie it to the diff, do not treat it as a finding.
...
The entire workflow only reviews: do not call apply_patch or write, format, auto-fix, or delete files; the runtime rejects those calls.`;
