...
export type ControlInputEventType = Exclude<
  InputEventType,
  typeof INPUT_EVENTS.TEXT | typeof INPUT_EVENTS.UNKNOWN | typeof INPUT_EVENTS.MOUSE | typeof INPUT_EVENTS.MOUSE_WHEEL | typeof INPUT_EVENTS.CURSOR_POSITION
>;
...
export type MouseInputEvent = {
  ...
  button: 'left' | 'middle' | 'right' | 'other'; // 报告对应的物理按键；未知按键归为 other。
...
export type MouseWheelInputEvent = {
  type: typeof INPUT_EVENTS.MOUSE_WHEEL; // 独立于鼠标点击/移动的垂直滚轮报告。
  direction: MouseWheelDirection; // 一个报告仅对应一个离散方向步长。
  column: number; // 终端屏幕 1-based 列坐标。
  row: number; // 终端屏幕 1-based 行坐标。
};
...
export type InputEvent = TextInputEvent | UnknownInputEvent | ControlInputEvent | MouseInputEvent | MouseWheelInputEvent | CursorPositionInputEvent;
