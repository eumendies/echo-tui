  ...
  const controller = new ComposerSubmissionController({
    ...
    command: {
      ...
      startFromText(text) {
        ...
        return options.startFromText ? options.startFromText(text) : {kind: 'not_matched'};
      }
    },
    reference: {
...
test('ComposerSubmissionController queues once during a turn and dispatches queued slash text once', async () => {
  const harness = createHarness({
    startFromText(text) {
      return text === '/status' ? {kind: 'handled'} : {kind: 'not_matched'};
    }
  });
  harness.appContext.setMcpBootstrapStatus('ready');
  ...
  await harness.controller.submitComposer();
  harness.appContext.composerContext.setText('later draft');
  harness.appContext.turnContext.finishAssistantTurn('');
  await Promise.all([
    harness.controller.dispatchPendingMessage(),
    harness.controller.dispatchPendingMessage()
  ]);
